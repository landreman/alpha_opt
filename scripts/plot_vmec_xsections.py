...
print()
print("usage: " + __file__ + " <woutXXX.nc>")
print("Include any additional arguments to save a PDF")

if len(sys.argv) < 2:
    print("Error! You must specify at least 1 argument: the woutXXX.nc file.")
    exit(1)

save_fig = len(sys.argv) > 2
...
if save_fig:
    print('Saving PDF')
    plt.savefig(filename[:-3] + "_xsections.pdf")
else:
    plt.show()
