...
def test_usable_space_pca():
    for space in ["Garabedian", "RealSpace"]:
        ...
        np.testing.assert_array_less(success_fraction, 1.0 + 1e-10)
        np.testing.assert_array_less(0.0, success_fraction)
        np.testing.assert_array_less(0, n_good_iota)
        np.testing.assert_array_less(n_good_iota, n_trials + 1)
        np.testing.assert_array_less(n_good_iota_fraction, 1.0 + 1e-10)
